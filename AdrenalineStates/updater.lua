...
function screen.flip()
	...
	if UPDATE_PORT:available() > 0 then
		...
		if os.message("Adrenaline States "..string.format("%X.%02X",major, minor).." is now available.\n".."Do you want to update the application?",1) == 1 then
			buttons.homepopup(0)
			
			local url = string.format("https://github.com/DavisDev/AdrenalineStates/releases/download/%s/AdrenalineStates.vpk", string.format("%X.%02X",major, minor))
			local path = "ux0:/data/AdrenalineStates/AdrenalineStates.vpk"
